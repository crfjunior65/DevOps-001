# Security Groups independentes (não referenciam outros SGs)
...
resource "aws_security_group" "bia-web" {
  ...
  tags = {
    Name = "Acesso Bia-Web DynamicBlock"
  }
}
...
# Security Groups que dependem de outros (criados depois)
    ...
    content {
      ...
      security_groups = [
        for sg_name in ingress.value["security_groups"] : 
          sg_name == "bia-alb" ? aws_security_group.bia-alb.id :
          sg_name == "bia-dev" ? aws_security_group.bia-dev.id : null
      ]
    }
  }
...
resource "aws_security_group" "bia-db" {
  ...
  dynamic "ingress" {
    ...
    content {
      ...
      security_groups = [
        for sg_name in ingress.value.security_groups : 
          sg_name == "bia-web" ? aws_security_group.bia-web.id :
          sg_name == "bia-ec2" ? aws_security_group.bia-ec2.id :
          sg_name == "bia-dev-mssql" ? aws_security_group.bia-dev-mssql.id :
          sg_name == "bia-build" ? aws_security_group.bia-build.id :
          sg_name == "bia-dev" ? aws_security_group.bia-dev.id : null
      ]
    }
  }
...
resource "aws_security_group" "windows-sg" {
  ...
  dynamic "ingress" {
    ...
    content {
      ...
      to_port     = ingress.value["to_port"]
      protocol    = ingress.value["protocol"]
      # cidr_blocks é opcional
      cidr_blocks = lookup(ingress.value, "cidr_blocks", [])
      # security_groups é opcional - usa referência direta
